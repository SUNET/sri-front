  ...
  render() {
    ...
    return (
      <div className={`filter-columns ${this.props.type}`}>
        <Dropdown>
          <Dropdown.Toggle as="span">{this.props.type === 'order' && <i className="icon-filter"></i>}</Dropdown.Toggle>
          <Dropdown.Menu>
            ...
            <div>
              {this.props.columns.map((column) => {
                ...
                if (this.props.column === 'organization' || this.props.column === 'roles') {
                  defaultValue =
                    this.state.filters[column.name] !== undefined ? this.state.filters[column.name] : false;
                  id = column.name;
                } else {
                  defaultValue =
                    this.state.filters[column.value] !== undefined ? this.state.filters[column.value] : false;
                  id = column.value;
                }
                return (
                  <FieldSwitch
