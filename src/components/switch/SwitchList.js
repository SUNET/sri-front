...
export class SwitchList extends React.Component {
  ...
  renderList() {
    let { switchs } = this.props;

    return (
      <tbody>
        {switchs &&
          switchs.switchs &&
          switchs.switchs.edges.map(({ node }) => {
            return (
              <this.ROW_COMPONENT
                key={node.id}
                switch={node}
...
export default createPaginationContainer(
  ...
  },
  {
    ...
    query: graphql`
      ...
      query SwitchListForwardQuery($count: Int!, $cursor: String, $orderBy: SwitchOrderBy) {
        ...SwitchList_switchs @arguments(count: $count, cursor: $cursor, orderBy: $orderBy)
      }
    `,
