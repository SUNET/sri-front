  ...
  renderList() {
    ...
    return (
      <tbody>
        {services &&
          services.services &&
          services.services.edges.map(({ node }) => {
            return (
              <this.ROW_COMPONENT
                key={node.id}
                service={node}
...
export default createPaginationContainer(
  ...
  },
  {
    ...
    query: graphql`
      ...
      query ServiceListForwardQuery($count: Int!, $cursor: String, $orderBy: ServiceOrderBy) {
        ...ServiceList_services @arguments(count: $count, cursor: $cursor, orderBy: $orderBy)
      }
    `,
