...
export class RouterList extends React.Component {
  MODEL_NAME = 'router';
  MODEL_LIST_NAME = 'routers';
  ROW_COMPONENT = ROW_COMPONENT;
  static propTypes = {
    routers: PropTypes.object,
  };
  ...
  renderList() {
    ...
    return (
      <tbody>
        {routers &&
          routers.routers &&
          routers.routers.edges.map(({ node }) => {
            return (
              <this.ROW_COMPONENT
                key={node.id}
                router={node}
...
export default createPaginationContainer(
  ...
  },
  {
    ...
    query: graphql`
      ...
      query RouterListForwardQuery($count: Int!, $cursor: String, $orderBy: RouterOrderBy) {
        ...RouterList_routers @arguments(count: $count, cursor: $cursor, orderBy: $orderBy)
      }
    `,
