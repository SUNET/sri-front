...
export class FirewallList extends React.Component {
  MODEL_NAME = 'firewall';
  MODEL_LIST_NAME = 'firewalls';
  ROW_COMPONENT = ROW_COMPONENT;
  static propTypes = {
    firewalls: PropTypes.object,
  };
  ...
  renderList() {
    let { firewalls } = this.props;

    return (
      <tbody>
        {firewalls &&
          firewalls.firewalls &&
          firewalls.firewalls.edges.map(({ node }) => {
            return (
              <this.ROW_COMPONENT
                key={node.id}
                firewall={node}
...
export default createPaginationContainer(
  ...
  },
  {
    ...
    query: graphql`
      ...
      query FirewallListForwardQuery($count: Int!, $cursor: String, $orderBy: FirewallOrderBy) {
        ...FirewallList_firewalls @arguments(count: $count, cursor: $cursor, orderBy: $orderBy)
      }
    `,
