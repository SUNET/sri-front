...
export class OpticalPathList extends React.Component {
  MODEL_NAME = 'opticalPath';
  MODEL_LIST_NAME = 'opticalPaths';
  ROW_COMPONENT = ROW_COMPONENT;
  static propTypes = {
    opticalPaths: PropTypes.object,
  };
  ...
  renderList() {
    let { opticalPaths } = this.props;

    return (
      <tbody>
        {opticalPaths &&
          opticalPaths.opticalPaths &&
          opticalPaths.opticalPaths.edges.map(({ node }) => {
            return (
              <this.ROW_COMPONENT
                key={node.id}
                opticalPath={node}
  ...
  render() {
    const { t, opticalPaths } = this.props;
...
export default createPaginationContainer(
  ...
  },
  {
    ...
    query: graphql`
      ...
      query OpticalPathListForwardQuery($count: Int!, $cursor: String, $orderBy: OpticalPathOrderBy) {
        ...OpticalPathList_opticalPaths @arguments(count: $count, cursor: $cursor, orderBy: $orderBy)
      }
    `,
