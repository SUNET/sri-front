...
export class OpticalNodeList extends React.Component {
  MODEL_NAME = 'opticalNode';
  MODEL_LIST_NAME = 'opticalNodes';
  ROW_COMPONENT = ROW_COMPONENT;
  static propTypes = {
    opticalNodes: PropTypes.object,
  };
  ...
  renderList() {
    let { opticalNodes } = this.props;

    return (
      <tbody>
        {opticalNodes &&
          opticalNodes.opticalNodes &&
          opticalNodes.opticalNodes.edges.map(({ node }) => {
            return (
              <this.ROW_COMPONENT
                key={node.id}
                opticalNode={node}
  ...
  render() {
    const { t, opticalNodes } = this.props;
...
export default createPaginationContainer(
  ...
  },
  {
    ...
    query: graphql`
      ...
      query OpticalNodeListForwardQuery($count: Int!, $cursor: String, $orderBy: OpticalNodeOrderBy) {
        ...OpticalNodeList_opticalNodes @arguments(count: $count, cursor: $cursor, orderBy: $orderBy)
      }
    `,
