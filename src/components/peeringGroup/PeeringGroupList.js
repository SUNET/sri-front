...
export class PeeringGroupList extends React.Component {
  MODEL_NAME = 'peeringGroup';
  MODEL_LIST_NAME = 'peeringGroups';
  ROW_COMPONENT = ROW_COMPONENT;
  static propTypes = {
    peeringGroups: PropTypes.object,
  };
  ...
  renderList() {
    let { peeringGroups } = this.props;

    return (
      <tbody>
        {peeringGroups &&
          peeringGroups.peeringGroups &&
          peeringGroups.peeringGroups.edges.map(({ node }) => {
            return (
              <this.ROW_COMPONENT
                key={node.id}
                peeringGroup={node}
  ...
  render() {
    const { t, peeringGroups } = this.props;
...
export default createPaginationContainer(
  ...
  },
  {
    ...
    query: graphql`
      ...
      query PeeringGroupListForwardQuery($count: Int!, $cursor: String, $orderBy: PeeringGroupOrderBy) {
        ...PeeringGroupList_peeringGroups @arguments(count: $count, cursor: $cursor, orderBy: $orderBy)
      }
    `,
