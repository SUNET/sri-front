...
export class OpticalFilterList extends React.Component {
  MODEL_NAME = 'opticalFilter';
  MODEL_LIST_NAME = 'opticalFilters';
  ROW_COMPONENT = ROW_COMPONENT;
  static propTypes = {
    opticalFilters: PropTypes.object,
  };
  ...
  renderList() {
    let { opticalFilters } = this.props;

    return (
      <tbody>
        {opticalFilters &&
          opticalFilters.opticalFilters &&
          opticalFilters.opticalFilters.edges.map(({ node }) => {
            return (
              <this.ROW_COMPONENT
                key={node.id}
                opticalFilter={node}
  ...
  render() {
    const { t, opticalFilters } = this.props;
...
export default createPaginationContainer(
  ...
  },
  {
    ...
    query: graphql`
      ...
      query OpticalFilterListForwardQuery($count: Int!, $cursor: String, $orderBy: OpticalFilterOrderBy) {
        ...OpticalFilterList_opticalFilters @arguments(count: $count, cursor: $cursor, orderBy: $orderBy)
      }
    `,
